...
export default function AddInventory() {
  ...
  function sendData(e) {
    ...
    axios
      .post("http://localhost:8411/inventory/add", newInventory)
      .then((response) => {
...
/*
  const validateAmpHrs = (ampHrs) => {
    ...
    if (!/^\d+(\.\d+)?$/.test(ampHrs) || parseFloat(ampHrs) < 0) {
      setAmpHrsError("Battery Ampers must be a non-negative whole or real number");
      return false;
    }
...
*/
  ...
  <input
    ...
    onChange={(e) => {
      ...
      // Validate Product ID as the user types
...
<div className="form-group">
  ...
  <input
    ...
    onChange={(e) => {
      ...
      // Validate Product Type as the user types
...
<div className="form-group">
  <label htmlFor="amp_hrs">Battery Ampiers</label>
  <input
    type="text"
    className="form-control"
    id="amp_hrs"
    placeholder="Enter Battery Ampers"
    value={amp_hrs}
    onChange={(e) => {
...
<div className="form-group">
  <label htmlFor="vehicle_man_year">Vehicle Manufactured Year</label>
  <input
    type="text"
    className="form-control"
    id="vehicle_man_year"
  ...
  {vehicleManYearError && <div className="text-danger">{vehicleManYearError}</div>}
        ...
        <Link to="/inventory" className="back-to-dashboard-link">
